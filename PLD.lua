...
local function twoHanded()
	...
	return weapon and
		(weapon.skill == 4 or weapon.skill == 6 or weapon.skill == 7 or weapon.skill == 8 or weapon.skill == 10 or weapon.skill == 12) or
		false
end
...
local function getRecasts()
	...
	Cocoon.recast = (type(spell_data) == 'table' and table.contains(spell_data, 547)) and
		math.ceil(spell_recast[547] / 60)
	...
	Refueling.recast = (type(spell_data) == 'table' and table.contains(spell_data, 530)) and
		math.ceil(spell_recast[530] / 60)
...
local function formatAbils(input, input_sh)
	...
	local validAbilities = {
		"Invincible", "Intervene", "Aggressor", "Berserk", "Chivalry", "Cocoon", "Cover", "Crusade", "Dark Arts",
		"Defender", "Divine Emblem", "Enlight", "Enlight II", "Fealty", "Holy Circle", "Light Arts", "Majesty",
		"Palisade", "Phalanx", "Rampart", "Refueling", "Reprisal", "Sentinel", "Sepulcher", "Shield Bash", "Stoneskin",
		"Sublimation", "Warcry"
	}

	local ab = {}
	ab['Invincible'] = Invincible
	ab['Intervene'] = Intervene
	ab['Aggressor'] = Aggressor
	ab['Berserk'] = Berserk
	ab['Chivalry'] = Chivalry
	ab['Cocoon'] = Cocoon
	ab['Cover'] = Cover
	ab['Crusade'] = Crusade
	ab['Dark Arts'] = DarkArts
	ab['Defender'] = Defender
	ab['Divine Emblem'] = DivineEmblem
	ab['Enlight'] = Enlight
	ab['Enlight II'] = Enlight
	ab['Fealty'] = Fealty
	ab['Holy Circle'] = HolyCircle
	ab['Light Arts'] = LightArts
	ab['Majesty'] = Majesty
	ab['Palisade'] = Palisade
	ab['Phalanx'] = Phalanx
	ab['Rampart'] = Rampart
	ab['Refueling'] = Refueling
	ab['Reprisal'] = Reprisal
	ab['Sentinel'] = Sentinel
	ab['Sepulcher'] = Sepulcher
	ab['Shield Bash'] = ShieldBash
	ab['Stoneskin'] = Stoneskin
...
function precast(spell)
	...
	elseif AutoMajesty and ((string.find(spell.english, 'Cur') and spell.type == 'WhiteMagic') or string.find(spell.english, 'Protect')) and not buffactive['Majesty'] and not buffactive['amnesia'] and Majesty.recast == 0 then
		if not double_majesty_fix then
			...
			cancel_spell()
			send_command('input /ja Majesty <me>;wait 1;input /ma \"' ..
				spell.english .. '\" ' .. spell.target.raw .. ';wait 1;gs c double_majesty_fix')
			return
		end
		...
		if windower.ffxi.get_spell_recasts()[112] and windower.ffxi.get_spell_recasts()[112] < 120 then
			if AutoDEmblem and not buffactive['amnesia'] and DivineEmblem.recast == 0 then
				if not double_divine_emblem_fix then
					...
					cancel_spell()
					send_command('input /ja "Divine Emblem" <me>;wait 1;input /ma Flash ' ..
						spell.target.raw .. ';wait 1;gs c double_divine_emblem_fix')
					return
				end
			end
			local main_sub = use_mainsub and { main = nil, sub = nil } or { main = "", sub = "" }
...
function midcast(spell)
	...
	local use_sird = (Mode == 'Tank' or Mode == 'Kite' or ((Mode == 'Auto' or Mode == 'DPS') and player_in_combat)) and
	...
	elseif spell.skill == "Enhancing Magic" then
		...
		local main_sub = string.find(spell.english, 'Protect') and use_mainsub and { main = nil, sub = nil } or
			{ main = "", sub = "" }
		equip(set_combine(base_set, main_sub))
...
function aftercast(spell)
	...
	if spell.english == 'Invincible' and InvTimer and not spell.interrupted then
		if player.equipment.legs == 'Cab. Breeches' or player.equipment.legs == 'Cab. Breeches +1' or player.equipment.legs == 'Cab. Breeches +2' or player.equipment.legs == 'Cab. Breeches +3' or player.equipment.legs == 'Cab. Breeches +4' then --these pieces extend Invincible by 10 seconds so we adjust accordingly
			send_command(
				'input /echo [Invincible] 40 seconds;wait 10;input /echo [Invincible] 30 seconds;wait 10;input /echo [Invincible] 20 seconds;wait 10;input /echo [Invincible] 10 seconds')
		else
			send_command(
				'input /echo [Invincible] 30 seconds;wait 10;input /echo [Invincible] 20 seconds;wait 10;input /echo [Invincible] 10 seconds')
		end
	elseif spell.english == 'Intervene' and IntTimer and not spell.interrupted then
		send_command(
			'input /echo [Intervene] 30 seconds;wait 10;input /echo [Intervene] 20 seconds;wait 10;input /echo [Intervene] 10 seconds')
