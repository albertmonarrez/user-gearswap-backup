...
SubWHMPage      =
'1'                     --[1-10/Off]	Sets your Macro page to any number from 1 to 10 (or Off) on file load or subjob change when subbing WHM.
SubRDMPage      =
'1'                     --[1-10/Off]	Sets your Macro page to any number from 1 to 10 (or Off) on file load or subjob change when subbing RDM.
SubBLMPage      =
'1'                     --[1-10/Off]	Sets your Macro page to any number from 1 to 10 (or Off) on file load or subjob change when subbing BLM.
SubSCHPage      =
...
-- Heads Up Display --
HUDposX         = 1102 --	X position for the HUD. 0 is left of the window, increasing this number will move it to the right.
HUDposY         = 970  --	Y position for the HUD. 0 is top of the window, increasing this number will move it downward.
...
notifications   = {
	...
	ReraiseReminder = false, --[true/false]	Displays an occasional reminder if Reraise is not up.
	TP3000          = true, --[true/false]	Displays a notification when you have 3000 TP.
	Damage          = false, --[true/false]	Displays your Weapon Skill, Skillchain, and Magic Burst damage.
	Food            = false, --[true/false]	Displays a notification when food wears off.
	...
	Reraise         = false, --[true/false]	Displays a notification when reraise wears off.
	Signet          = false, --[true/false]	Displays a notification when Signet/Sanction/Sigil/Ionis wears off.
...
sub   = {
	...
	WHM = {
		...
		Abil01_sh = "",
		Abil02 = "Blaze of Glory",
		Abil02_sh = "BlazeGlory",
		Abil03 = "Dematerialize",
		Abil03_sh = "Dematrialz",
		Abil04 = "Ecliptic Attrition",
		Abil04_sh = "Ecliptic",
		Abil05 = "Life Cycle",
		Abil05_sh = "",
		Abil06 = "Divine Seal",
	...
	SCH = {
		...
		Abil05_sh = "",
		Abil06 = "Sublimation",
		Abil06_sh = "Sublmation",
	},
	--GEO/other
	OTH = {
		Abil01 = "Radial Arcana",
		Abil01_sh = "",
		Abil02 = "Mending Halation",
		Abil02_sh = "Mending",
		Abil03 = "Blaze of Glory",
		Abil03_sh = "BlazeGlory",
		Abil04 = "Dematerialize",
		Abil04_sh = "Dematrialz",
		Abil05 = "Ecliptic Attrition",
		Abil05_sh = "Ecliptic",
		Abil06 = "Life Cycle",
		Abil06_sh = "",
	}
...
function get_sets()
	...
	sets.fast_cast         = {
		...
		legs = inv.geomancy_pants,                                                                                                                       --15%
		feet = inv.merlinc_fastcast_feet,                                                                                                                --6%
		neck = inv.baetyl_pendant,                                                                                                                       --4%
		waist = inv.embla_sash,                                                                                                                          --5%
		left_ear = inv.malignance_earring,                                                                                                               --4%
		right_ear = inv.alabaster_earring,
		left_ring = inv.weatherspoon_ring,                                                                                                               --2%
		right_ring = inv.kishar_ring,                                                                                                                    --4%
	...
	sets.healing           = {
		...
		legs = inv.gyve_trousers,
		feet = "Vanya Clogs",        --5
		neck = inv.nodens_gorget,    --5
		left_ear = inv.mendicants_earring, --5
		right_ear = inv.regal_earring,
...
local function formatAbils(input, input_sh)
	...
	local validAbilities = {
		"Bolster", "Widened Compass", "Blaze of Glory", "Collimated Fervor", "Concentric Pulse", "Convert", "Dark Arts",
		"Dematerialize", "Divine Seal", "Ecliptic Attrition", "Elemental Seal", "Entrust", "Full Circle",
		"Lasting Emanation", "Life Cycle", "Light Arts", "Mending Halation", "Radial Arcana", "Sublimation",
		"Theurgic Focus"
	}
	...
	ab['Blaze of Glory'] = BlazeofGlory
	ab['Collimated Fervor'] = CollimatedFervor
	ab['Concentric Pulse'] = ConcentricPulse
	ab['Convert'] = Convert
	ab['Dark Arts'] = DarkArts
	ab['Dematerialize'] = Dematerialize
	ab['Divine Seal'] = DivineSeal
	ab['Ecliptic Attrition'] = EclipticAttrition
	ab['Elemental Seal'] = ElementalSeal
	ab['Entrust'] = Entrust
	ab['Full Circle'] = FullCircle
	ab['Lasting Emanation'] = LastingEmanation
	ab['Life Cycle'] = LifeCycle
	ab['Light Arts'] = LightArts
	ab['Mending Halation'] = MendingHalation
	ab['Radial Arcana'] = RadialArcana
	ab['Sublimation'] = Sublimation
	...
	elseif command == 'ClearNotifications' then --these reset the Notifications display back to a basic state
		...
		if TownZones:contains(world.area) or windower.ffxi.get_info().mog_house then
			hud_noti_shdw:text(player.name ..
				...
				'/' .. (player.sub_job_level and player.sub_job .. player.sub_job_level or "---"))
...
function precast(spell)
	...
	if TransportLock and transport_spells:contains(spell.en) and transport_locked then
		...
		windower.add_to_chat(8,
			('[Notice] '):color(39) .. (spell.name):color(1) ..
			(' cancelled. Unlocked for 3 min or until zone.'):color(8))
		if AlertSounds then
	...
	elseif string.find(spell.english, 'Geo-') and LuopanActive and AutoFullCircle and FullCircle.recast < 2 then
		...
		if not double_full_circle_fix then
			...
			cancel_spell()
			send_command('input /ja "Full Circle" <me>;wait 1;input /ma \"' ..
				spell.english .. '\" ' .. spell.target.raw .. ';wait 1;gs c double_full_circle_fix')
			return
		end
		...
		if AutoEntrust and Entrust.recast == 0 and spell.target.ispartymember and spell.target.type ~= 'SELF' then
			...
			elseif UseEntrust then
				...
				if not double_entrust_fix then
					...
					cancel_spell()
					send_command('input /ja "Entrust" <me>;wait 1;input /ma \"' .. spell.english ..
						'\" ' .. spell.target.raw)
...
function aftercast(spell)
		...
		if string.find(spell.english, 'Indi-') then
			if buffactive['Entrust'] then
				...
				if is_npc then
					...
					EntrustTarget =
						Target -- this is the main way we determine when the Entrusted spell has worn off another player
	...
	elseif spell.english == 'Bolster' and BolTimer and not spell.interrupted then
		if player.equipment.body == 'Bagua Tunic' or player.equipment.body == 'Bagua Tunic +1' or player.equipment.body == 'Bagua Tunic +2' or player.equipment.body == 'Bagua Tunic +3' or player.equipment.body == 'Bagua Tunic +4' then --these pieces extend Bolster by 30 seconds so we adjust accordingly
			send_command(
				'input /echo [Bolster] 3:30;wait 30;input /echo [Bolster] 3:00;wait 30;input /echo [Bolster] 2:30;wait 30;input /echo [Bolster] 2:00;wait 30;input /echo [Bolster] 1:30;wait 30;input /echo [Bolster] 1:00;wait 30;input /echo [Bolster] 0:30;wait 10;input /echo [Bolster] 0:20;wait 10;input /echo [Bolster] 0:10')
		else
			send_command(
				'input /echo [Bolster] 3:00;wait 30;input /echo [Bolster] 2:30;wait 30;input /echo [Bolster] 2:00;wait 30;input /echo [Bolster] 1:30;wait 30;input /echo [Bolster] 1:00;wait 30;input /echo [Bolster] 0:30;wait 10;input /echo [Bolster] 0:20;wait 10;input /echo [Bolster] 0:10')
		end
	elseif spell.english == 'Widened Compass' and WCTimer and not spell.interrupted then
		send_command(
			'input /echo [Widened Compass] 1:00;wait 30;input /echo [Widened Compass] 0:30;wait 10;input /echo [Widened Compass] 0:20;wait 10;input /echo [Widened Compass] 0:10')
	end
	choose_set()
...
windower.register_event('gain buff', function(buff)
	...
	elseif buff == 612 then                        --Colure Active
		hud_indi_spell_shdw:text(format24((IndiSpell == 'None' and 'Unknown' or IndiSpell) ..
			(IndiTotal and IndiTotal or '') .. (IndiSuffix and IndiSuffix or '')))
		hud_indi_spell:text(format24((IndiSpell == 'None' and 'Unknown' or IndiSpell) ..
			(IndiTotal and IndiTotal or '') .. (IndiSuffix and IndiSuffix or '')))
		local c = buffactive['Bolster'] and color.bubble.bolster or color.bubble.normal
		hud_indi_spell:color(c.r, c.g, c.b)
...
windower.register_event('prerender', function()
	...
	if current_time - last_poll >= 1 / PollingRate then
		...
		elseif buffactive['stun'] and notifications.Stun and Alive then
			...
			if SIL == '    ' and PAR == '     ' and PLG == '     ' and CUR == '     ' and AMN == '     ' and TNT == '     ' and ENC == '     ' then
				...
				hud_debuffs_shdw:text(' ' ..
					...
					PAR:text_strip_format() ..
					PLG:text_strip_format() .. SIL:text_strip_format() .. TNT:text_strip_format())
			end
		end
		...
		if pet.isvalid then
			...
			hud_geo_spell_shdw:text(format24((GeoSpell == 'None' and 'Unknown' or GeoSpell) ..
				(tempTotal and tempTotal or '') .. (GeoSuffix and GeoSuffix or '')))
			hud_geo_spell:text(format24((GeoSpell == 'None' and 'Unknown' or GeoSpell) ..
				(tempTotal and tempTotal or '') .. (GeoSuffix and GeoSuffix or '')))
			hud_geo_spell:color(c.r, c.g, c.b)
			LuopanActive = true
		...
		if buffactive['Colure Active'] then
			...
			hud_indi_spell_shdw:text(format24((IndiSpell == 'None' and 'Unknown' or IndiSpell) ..
				(tempTotal and tempTotal or '') .. (IndiSuffix and IndiSuffix or '')))
			hud_indi_spell:text(format24((IndiSpell == 'None' and 'Unknown' or IndiSpell) ..
				(tempTotal and tempTotal or '') .. (IndiSuffix and IndiSuffix or '')))
			hud_indi_spell:color(c.r, c.g, c.b)
		end
